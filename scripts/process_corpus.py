...
from legal_similarity.stages import process_corpus_once  # type: ignore[import-not-found]


def main() -> None:
    parser = argparse.ArgumentParser(description="Stage 2: process corpus once and save tokens JSONL.")
    parser.add_argument(
        "--corpus",
        default=str(ROOT / "data" / "corpus" / "legal_docs"),
        help="Corpus folder containing .txt/.md/.docx files",
    )
    parser.add_argument(
        "--output",
        default=str(ROOT / "outputs" / "processed" / "processed_docs.jsonl"),
        help="Processed token JSONL output",
    )
    parser.add_argument("--k", type=int, default=3, help="Word shingle size used later")
